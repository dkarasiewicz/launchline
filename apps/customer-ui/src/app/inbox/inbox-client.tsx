...
import { LineaChat } from '@launchline/ui/components/inbox/linea-chat';
...
function InboxPageContent() {
  ...
  const items = useMemo(() => {
    ...
    return threadsData.threads.threads
      .filter((thread) => {
        if (!thread.isInboxThread) return false;
        const normalizedThreadStatus =
          (thread.status || '').toLowerCase() === 'archived'
            ? 'archived'
            : 'regular';
        if (normalizedThreadStatus === 'archived' && !showResolved) {
          return false;
        }
        return true;
      })
  ...
  }, [threadsData, loading, showResolved]);
    ...
    <div className="flex h-screen bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {selectedItem ? (
          ...
          // Linear is connected - show Linea Chat
          <LineaChat />
        )}
      </div>
