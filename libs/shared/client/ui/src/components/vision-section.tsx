...
const stages = [
  ...
  },
  {
    ...
    description: "Understands how your team, product, and users behave over time.",
    icon: Brain,
    active: false,
  },
  {
      ...
      <div className="mx-auto max-w-4xl px-6">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-base text-muted-foreground max-w-2xl mx-auto leading-relaxed">
            Products today are built across dozens of tools, teams, and decisions. PMs are expected to keep everything
            aligned — but the system itself doesn't learn. Launchline is building the missing layer: a product copilot
            that understands <span className="text-accent">context</span>,{" "}
            <span className="text-status-info">history</span>, and{" "}
            <span className="text-status-success">tradeoffs</span> — not just tickets.
          </p>
        </div>
                ...
                <div className="flex items-center gap-2 mb-3">
                  <span
                    className={`text-[10px] font-medium uppercase tracking-widest ${
                      stage.active ? "text-accent" : index === 1 ? "text-status-info" : "text-muted-foreground/80"
                    }`}
                  >
                    {stage.label}
                  </span>
                  {stage.active && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-accent/20 text-accent">Current</span>
                  )}
                </div>
                <div className="flex items-center gap-3 mb-2">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                      stage.active ? "bg-accent/15" : index === 1 ? "bg-status-info/15" : "bg-secondary"
                    }`}
                  >
                    <stage.icon
                      className={`h-4 w-4 ${
                        stage.active ? "text-accent" : index === 1 ? "text-status-info" : "text-muted-foreground"
                      }`}
                    />
                  </div>
                  <h3 className={`text-sm font-medium ${stage.active ? "text-foreground" : "text-foreground/90"}`}>
                    {stage.title}
                  </h3>
                </div>
                <p
        ...
        <div className="mt-12 text-center">
          <p className="text-sm text-muted-foreground/80 leading-relaxed">
            ...
            <span className="text-muted-foreground">execution. Inbox → decisions → unblock → ship.</span>
          </p>
        </div>
      </div>
    </section>
