...
const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    icon: <img src="/linear.png" alt="Linear" className="w-6 h-6" />,
    connectable: true,
  },
];
...
export function IntegrationsStep({
  ...
  const { data: integrationsData, refetch } = useQuery<{
    integrations: {
      integrations: { id: string; type: string; status: string }[];
    };
  }>(INTEGRATIONS_QUERY, { fetchPolicy: 'network-only' });

  const connected = useMemo(() => {
    const active = new Set(
      (integrationsData?.integrations?.integrations || [])
        .filter((integration) => integration.status.toLowerCase() === 'active')
        .map((integration) => integration.type.toLowerCase()),
    );

    return INTEGRATIONS.filter((integration) => active.has(integration.id)).map(
  ...
  useEffect(() => {
    ...
    setSkipped((prev) => prev.filter((id) => !connected.includes(id)));
  }, [connected]);
  ...
  const handleConnect = (integrationId: ConnectedIntegration) => {
    ...
    window.location.href = `${API_BASE}/integrations/oauth/${integrationId}/init?redirect_url=${redirectUrl}`;
  };
  ...
  const canContinue =
    connected.length >= 1 || skipped.length === INTEGRATIONS.length;
                  ...
                  {isConnected ? (
                    ...
                    <>
                      ...
                      <Button
                        ...
                        disabled={isConnecting || !integration.connectable}
