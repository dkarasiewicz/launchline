 ...
 *
 * Renders AI messages with rich markdown formatting.
 */
...
const CodeHeader: FC<CodeHeaderProps> = ({ language, code }) => {
  ...
  return (
    <div className="mt-4 flex items-center justify-between gap-4 rounded-t-lg bg-muted-foreground/15 px-4 py-2 font-semibold text-foreground text-sm dark:bg-muted-foreground/20">
      <span className="lowercase [&>span]:text-xs">{language}</span>
      <button
        onClick={onCopy}
        className="flex items-center gap-1 text-xs hover:text-foreground/80"
      >
        {!isCopied && <CopyIcon className="h-3 w-3" />}
        {isCopied && <CheckIcon className="h-3 w-3" />}
...
const defaultComponents = memoizeMarkdownComponents({
  ...
  pre: ({ className, ...props }) => (
    <pre
      className={cn(
        'overflow-x-auto rounded-b-lg bg-zinc-900 p-4 text-white',
        className,
      )}
      {...props}
    />
  ...
  code: function Code({ className, ...props }) {
    ...
    return (
      <code
        className={cn(
          !isCodeBlock &&
            'rounded border bg-muted px-1 py-0.5 font-mono text-sm',
          className,
        )}
        {...props}
      />
