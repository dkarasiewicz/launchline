...
export type InboxItemStatus = "new" | "pending" | "actioned" | "auto-resolved" | "closed" | "dismissed"
...
export interface InboxAction {
  ...
  icon?: string
  toolType?: "assign" | "clarify" | "linear-status" | "priority" | "slack" | "resolve" | "dismiss"
}
...
export const MOCK_INBOX_ITEMS: InboxItem[] = [
  {
    ...
    summary: "PR hasn't progressed for 3 days. Comments indicate confusion about the API contract.",
    ...
    messages: [
      ...
      },
      {
        ...
        timestamp: new Date(Date.now() - 10 * 60 * 1000),
        actions: [
          { id: "assign", label: "Assign Owner", variant: "default", toolType: "assign" },
          { id: "clarify", label: "Ask for Clarification", variant: "outline", toolType: "clarify" },
          { id: "priority", label: "Adjust Priority", variant: "outline", toolType: "priority" },
          { id: "slack", label: "Discuss in Slack", variant: "outline", toolType: "slack" },
        ],
      },
    ],
    executionLogs: [
  ...
  },
  {
    ...
    summary: "Work continues on dependent feature while blocker unresolved. Potential rework risk.",
    ...
    messages: [
      ...
      },
      {
        ...
        role: "agent",
        type: "action-buttons",
        content: "Recommendation: Either pause Feature B or prioritize unblocking Feature A.",
        timestamp: new Date(Date.now() - 30 * 60 * 1000),
        actions: [
          { id: "pause-b", label: "Pause Feature B", variant: "default", toolType: "linear-status" },
          { id: "reassign", label: "Reassign Resources", variant: "outline", toolType: "assign" },
          { id: "dismiss", label: "Dismiss Risk", variant: "outline", toolType: "dismiss" },
        ],
      },
    ],
    executionLogs: [
  ...
  },
  {
    ...
    summary: "Auth module coverage dropped from 85% to 72% after recent changes.",
    ...
    messages: [
      ...
      },
      {
        ...
        timestamp: new Date(Date.now() - 45 * 60 * 1000),
        actions: [
          { id: "create-ticket", label: "Create Test Ticket", variant: "default", toolType: "linear-status" },
          { id: "assign", label: "Assign to Author", variant: "outline", toolType: "assign" },
          { id: "dismiss", label: "Dismiss", variant: "outline", toolType: "dismiss" },
        ],
      },
    ],
    executionLogs: [
  ...
  },
  {
    ...
    summary: "Your weekly project update for MVP Core is ready for review and posting.",
    ...
    messages: [
      ...
      },
      {
        ...
        timestamp: new Date(Date.now() - 60 * 60 * 1000),
        actions: [
          { id: "post-linear", label: "Post to Linear", variant: "default", toolType: "linear-status" },
          { id: "post-slack", label: "Post to Slack", variant: "outline", toolType: "slack" },
  ...
  },
  {
    ...
    summary: "Database migration failing in staging environment. Assigned to Sarah.",
    ...
    messages: [
      ...
      },
      {
        ...
        confirmationData: { action: "assign", value: "sarah", label: "Sarah Kim" },
      },
    ],
    executionLogs: [
      {
      ...
      { id: "log-6", timestamp: new Date(Date.now() - 90 * 60 * 1000), actor: "user", action: "Assigned to Sarah" },
      {
        id: "log-7",
  ...
  },
  {
    ...
    executionLogs: [
      ...
      { id: "log-10", timestamp: new Date(Date.now() - 23 * 60 * 60 * 1000), actor: "user", action: "Posted to Slack" },
    ],
  },
]
...
interface InboxContextType {
  ...
  updateMessage: (itemId: string, messageId: string, updates: Partial<InboxMessage>) => void
  addMessage: (itemId: string, message: Omit<InboxMessage, "id" | "timestamp">) => void
  addExecutionLog: (itemId: string, log: Omit<ExecutionLog, "id" | "timestamp">) => void
}
...
export function InboxProvider({ children }: { children: ReactNode }) {
  ...
  const updateMessage = (itemId: string, messageId: string, updates: Partial<InboxMessage>) => {
    setItems((prev) =>
      prev.map((item) =>
        item.id === itemId
          ? {
              ...item,
              messages: item.messages.map((msg) => (msg.id === messageId ? { ...msg, ...updates } : msg)),
            }
          : item,
      ),
    )
  ...
  const addMessage = (itemId: string, message: Omit<InboxMessage, "id" | "timestamp">) => {
    ...
    setItems((prev) =>
      prev.map((item) => (item.id === itemId ? { ...item, messages: [...item.messages, newMessage] } : item)),
    )
    if (selectedItem?.id === itemId) {
      setSelectedItem((prev) => (prev ? { ...prev, messages: [...prev.messages, newMessage] } : null))
    }
  }

  const addExecutionLog = (itemId: string, log: Omit<ExecutionLog, "id" | "timestamp">) => {
    ...
    setItems((prev) =>
      prev.map((item) => (item.id === itemId ? { ...item, executionLogs: [...item.executionLogs, newLog] } : item)),
    )
    if (selectedItem?.id === itemId) {
      setSelectedItem((prev) => (prev ? { ...prev, executionLogs: [...prev.executionLogs, newLog] } : null))
    }
  }
